...
def add_angle_sin_cos_features(df, angle_cols=("yaw", "pitch", "roll")):
	...
	Angles are circular, so using sin/cos is safer than using raw degrees directly.
	"""
	df = df.copy()
	angle_feature_cols = []
...
def resample_attempt_to_fixed_steps(df_attempt, time_col, sensor_cols, n_steps=500):
	"""Resample one complete attempt to exactly n_steps evenly spaced time points.

	This does NOT take the first 500 original rows. It uses the full attempt duration
	and compresses or stretches that entire climb into 500 normalized temporal points.
	A 20-second attempt, 40-second attempt, and 80-second attempt all become 500 steps,
	but each still represents the full climb from start to finish.
	"""
...
def build_tcn_dataset(full_df, attempt_id_col, time_col, sensor_cols, label_col, n_steps=500):
...
def standardize_train_test_sequences(X_train, X_test):
	"""Fit a StandardScaler on the training sequences only and transform train/test."""
	if X_train.ndim != 3 or X_test.ndim != 3:
		raise ValueError("X_train and X_test must both have shape (attempts, time_steps, channels)")
